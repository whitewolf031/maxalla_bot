...
def register_driver_handlers(bot: telebot.TeleBot):
    ...
    def my_status(message):
        ...
        my = next((s for s in today_statuses
                   if s.get('driver') and
                   api_client.get_driver_by_telegram(message.from_user.id) and
                   str(api_client.get_driver_by_telegram(message.from_user.id).get('id')) == str(s.get('driver'))),
                  None)
    ...
    def all_drivers_info(message):
        ...
        text = f"🚌 <b>Mahalla liniya haydovchilari:</b>\n\n"
        for i, d in enumerate(drivers, 1):
            text += (
                f"━━━━━━━━━━━━━━━━\n"
                f"{i}. 👤 <b>{d['full_name']}</b>\n"
    ...
    def today_schedule(message):
        ...
        for i, d in enumerate(working, 1):
            ...
            mov = f"\n    🚗 {d.get('moving_direction', '')}" if d.get('moving_direction') else ""
            text += (
    ...
    def location_info(call):
        ...
        if not statuses:
            ...
            for d in statuses:
                text += f"🚗 <b>{d['driver_name']}</b> | {d['car_name']} | <code>{d['car_number']}</code>\n📞 {d['driver_phone']}\n\n"
    ...
    def moving_from_mahalla(message):
        ...
        result = api_client.update_driver_location(
            message.from_user.id, 'mahalla', 'mahalla_to_oydin'
        )
    ...
    def moving_to_oydin(message):
        ...
        driver = middlewares.get_driver(message.from_user.id)
        result = api_client.update_driver_location(
            message.from_user.id, 'oydin', 'mahalla_to_oydin'
        )
    ...
    def moving_direction_cb(call):
        ...
        direction_texts = {
            'mahalla_to_oydin': f"🚗 Mahalladan Oydin tomon yo'lga chiqdi!",
            'oydin_to_mahalla': f"↩️ Oydindan Mahalla tomon qaytmoqda!",
        }
